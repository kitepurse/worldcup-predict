...
def layer2_ai_analysis(team_a, team_b, stats_a, stats_b, h2h, data1, squads=None):
    """AI 推理分析 — 接收 Elo+Poisson 结果做参考，可选 squad 数据"""
    ...
    squad_b = (squads or {}).get(team_b, {})
    has_squad = (
        squad_a.get("source") not in (None, "unavailable")
        and squad_b.get("source") not in (None, "unavailable")
        and squad_a.get("players") and squad_b.get("players")
    )
    ...
    if has_squad:
        ...
        key_a = get_key_players(squad_a, 5)
        key_b = get_key_players(squad_b, 5)

        def _fmt_players(plist):
            lines = []
            for p in plist:
                pos = p.get("position_cn", p.get("position", ""))
                age = p.get("age", "?")
                lines.append(f"- {p['name']} ({pos}, {age}岁)")
            return "\n".join(lines)

        ssi_a = squad_a.get("ssi", "?")
        ssi_b = squad_b.get("ssi", "?")

        squad_section = f"""
### 2026世界杯官方大名单（真实数据）
{cn_a} 核心球员 (阵容强度SSI:{ssi_a}/100):
{_fmt_players(key_a)}

{cn_b} 核心球员 (阵容强度SSI:{ssi_b}/100):
{_fmt_players(key_b)}
...
"""
        ...
        # 伤病信息
        injuries = ""
        for side, sq in [(cn_a, squad_a), (cn_b, squad_b)]:
            inj = (sq.get("injuries") or [])
            if inj:
                injuries += f"\n{side} 伤病/缺阵: " + ", ".join(
                    [f"{i.get('player','?')}({i.get('status','?')})" for i in inj]
                )
        if injuries:
            squad_section += injuries + "\n"

        player_rule = ("- **球员引用规则**：分析中可以且仅可以引用上述大名单中的球员。"
                       "不得编造不在名单中的人（如已退役/未入选的球员）。"
                       "如有伤病标注，需考虑缺阵影响。\n")
    else:
        player_rule = ("- **禁止提及具体球员姓名！** 你没有各队最新大名单数据，"
                       "提及球员名字必然出错。请用\"前锋群\"\"中场核心\"\"后防线\"等泛指。\n")
...
### 重要提示（必须遵守）
- 这是2026年48队世界杯，存在大量实力悬殊场次
{player_rule}- **强制性规则**：如果两队Elo差距>400，TOP1比分必须≥3球差距（如4:0、5:1、3:0）
